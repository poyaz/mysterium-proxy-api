...
describe('MystProviderProxyService', () => {
  ...
  describe(`Connect vpn provider and create new proxy`, () => {
    ...
    beforeEach(() => {
      ...
      outputDownstreamProxyModel = new ProxyDownstreamModel({
        ...
        country: outputVpnProviderIsRegister.country,
        type: ProxyTypeEnum.MYST,
  ...
  describe(`Disconnect vpn provider and remove any proxy on provider`, () => {
    let inputId: string;
    let outputRunnerDownstreamModel: RunnerModel<[MystIdentityModel, VpnProviderModel]>;
    let outputDownstreamProxyModel: ProxyDownstreamModel;
    let outputRunnerUpstreamModel: RunnerModel<[MystIdentityModel, VpnProviderModel]>;
    ...
    beforeEach(() => {
      inputId = '77777777-7777-7777-7777-777777777777';
    ...
    it(`Should error disconnect vpn provider and remove any proxy on provider when get proxy upstream list`, async () => {
      ...
      expect((<FilterModel<ProxyUpstreamModel>>proxyService.getAll.mock.calls[0][0]).getCondition('proxyDownstream')).toEqual<FilterInstanceType<ProxyUpstreamModel> & { $opr: FilterOperationType }>({
        $opr: 'eq',
        proxyDownstream: [
          defaultModelFactory<ProxyDownstreamModel>(
            ProxyDownstreamModel,
    ...
    it(`Should error disconnect vpn provider and remove any proxy on provider when remove proxy list`, async () => {
      ...
      expect(proxyService.remove).toHaveBeenCalledTimes(1);
      expect(proxyService.remove).toHaveBeenNthCalledWith(1, outputProxyUpstreamModel.id);
      expect(error).toBeInstanceOf(UnknownException);
    });

    it(`Should error disconnect vpn provider and remove any proxy on provider when disconnect provider`, async () => {
      proxyService.getAll.mockResolvedValue([null, [outputProxyUpstreamModel], 1]);
      proxyService.remove.mockResolvedValue([null, null]);
      vpnProviderService.down.mockResolvedValue([new UnknownException()]);
      ...
      expect(proxyService.remove).toHaveBeenCalledTimes(1);
      expect(proxyService.remove).toHaveBeenNthCalledWith(1, outputProxyUpstreamModel.id);
      expect(vpnProviderService.down).toHaveBeenCalled();
      expect(vpnProviderService.down).toHaveBeenCalledWith(inputId);
      expect(error).toBeInstanceOf(UnknownException);
    });

    it(`Should successfully disconnect vpn provider and remove any proxy on provider`, async () => {
      proxyService.getAll.mockResolvedValue([null, [outputProxyUpstreamModel], 1]);
      proxyService.remove.mockResolvedValue([null, null]);
      vpnProviderService.down.mockResolvedValue([null, null]);
